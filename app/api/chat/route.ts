...
export async function POST(request: Request) {
  ...
  if (!apiKey) {
    return NextResponse.json({ error: '缺少SiliconFlow API密钥' }, { status: 500 });
  }
  ...
  try {
                  ...
                  if (!isControllerClosed) {
                    ...
                    controller.enqueue(
                      new TextEncoder().encode(
                        `data: ${JSON.stringify({ error: '解析数据错误' })}\n\n`
                      )
                    );
                  }
                }
    ...
    if (error.name === 'AbortError') {
      return NextResponse.json({ error: 'API请求超时，请重试' }, { status: 504 });
    }

    return NextResponse.json({ error: '调用AI模型失败，请重试' }, { status: 500 });
  }
}
